...
`affinity.user_affinity` (`- username: backend`) is a **login hint**, not a pin. On `AuthenticateByName`, HAP tries that user's preferred backend first among **eligible** backends unless a **live token** already pins the session. Leftover DeviceId / anon / cookie / least-loaded placement does not block the hint. Operator disable, health eligibility, and fail-closed still apply. `POST /hap/users/by-name/{username}/unpin` deletes that username's token rows (also matching `/Users/Public` ids, and wiped-identity orphans when those ids resolve) and **all DeviceId pins on those backends**. A still-valid Jellyfin token in the client can re-pin if the next hop is not a login — sign out, unpin, then sign in. See [config.md](config.md).
