...
- Token affinity wins; DeviceId is fallback; header-less media may follow a live session (cookie if that backend has a token/DeviceId, else IP-only glue); short-lived IP+User-Agent glue last. Unauthenticated login may retry other eligible backends on hop timeout or 401.
